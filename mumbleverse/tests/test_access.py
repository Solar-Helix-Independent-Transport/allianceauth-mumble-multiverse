...
class TestAccessPerms(TestCase):
    ...
    def setUp(cls):
        ...
        cls.access_perm = Permission.objects.get_by_natural_key(
            'basic_access', 'mumbleverse', 'general')
        cls.all_servers_perm = Permission.objects.get_by_natural_key(
            'global_access', 'mumbleverse', 'general')

        cls.server_1_no_perms_at_all = MumbleverseServer.objects.create(
            name="server 1 test",
        )

        cls.server_2_with_perms = MumbleverseServer.objects.create(
            name="server 2 test",
        )

    def test_no_perms(self):
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user1).count(), 0)
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user2).count(), 0)
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user3).count(), 0)
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user4).count(), 0)
    ...
    def test_all_perms(self):
        ...
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user1).count(), 2)
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user2).count(), 0)
        self.assertEqual(
            MumbleverseServer.objects.visible_to(self.user3).count(), 0)
    ...
    def test_state_perms_u1(self):
        ...
        self.assertIn(
            ...
            MumbleverseServer.objects.visible_to(self.user1)
        )
    ...
    def test_corp_perms_alli_1(self):
        ...
        self.assertIn(
            self.server_2_with_perms,
            MumbleverseServer.objects.visible_to(self.user2)
        )
        self.assertNotIn(
            self.server_1_no_perms_at_all,
            MumbleverseServer.objects.visible_to(self.user2)
    ...
    def test_can_access_func_u1(self):
        ...
        self.assertTrue(
            MumbleverseServer.user_can_access_server(self.user1, self.server_2_with_perms)
        )
        self.assertFalse(
            MumbleverseServer.user_can_access_server(self.user1, self.server_1_no_perms_at_all)
        )
    ...
    def test_can_access_func_u1_guild(self):
        ...
        self.assertTrue(
            MumbleverseServer.user_can_access_server(self.user1, self.server_2_with_perms.id)
        )
        self.assertFalse(
            MumbleverseServer.user_can_access_server(self.user1, self.server_1_no_perms_at_all.id)
        )
